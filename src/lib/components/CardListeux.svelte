...
<div class="card">
	<div class="headerInfo {$selectedListe === 1 ? 'styleListe1' : 'styleListe2'}">
		<img
			src="https://churros.inpt.fr/storage/{selectedUser.member.pictureFile}"
			alt="profile-pic"
		/>
		<div class="identity">
			<div class="name">
	...
	<div class="clubs">
		{#each selectedUser.member.groups as club}
			<Pills
				class={$selectedListe === 1 ? 'styleListe1' : 'styleListe2'}
				content={club.group.name}
			/>
		{/each}
	</div>
	<div class="actions">
		<Button type="redirect" class={$selectedListe === 1 ? 'styleListe1' : 'styleListe2'}
			>Profil Churros</Button
		>
